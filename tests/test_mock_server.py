...
class TestMessageHandling:
    ...
    def test_uid_registration_format(self):
        """Test UID registration message format (real server protocol)."""
        uid_message = {"command": "UID", "message": "test_user_123"}

        assert uid_message["command"] == "UID"
        assert uid_message["message"] == "test_user_123"
...
class TestMessageFormatValidation:
    ...
    @pytest.mark.parametrize(
        "command,message,expected_valid",
        [
